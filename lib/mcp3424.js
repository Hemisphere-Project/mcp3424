...
MCP3424 = (function() {
  ...
  //MCP3424.prototype.readRate = 10; // in milliseconds
 // MCP3424.prototype.oldChannel = 100;
  ...
  MCP3424.prototype.setChannel = function(chan) {
    var command;
    ...
    this.wire.readBytes(adcConfig, 4, function(err, res) {
      ...
      if(statusbyte & MCP342X_BUSY)
      	return callback("channel busy");
			...
			result = DIVIDER_RATIO * LSB[self.res] * (result / Math.pow(2,self.gain) / 1000000);
			//console.log(result);
        ...
        self._nextChannel();
        return self.setChannel(self.currChannel);
      } else {
        return "err";
      }*/
